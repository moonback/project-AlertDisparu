...
interface CardProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  variant?: 'default' | 'elevated' | 'outlined' | 'flat' | 'glass' | 'glass-strong';
  interactive?: boolean;
}
...
export const CardHeader: React.FC<CardProps> = ({ children, className, ...props }) => {
  return (
    <div className={cn('px-6 py-4 border-b border-white/20', className)} {...props}>
      {children}
    </div>
  );
};
...
export const CardFooter: React.FC<CardProps> = ({ children, className, ...props }) => {
  return (
    <div className={cn('px-6 py-4 border-t border-white/20 bg-white/10 rounded-b-xl', className)} {...props}>
      {children}
    </div>
  );
};
